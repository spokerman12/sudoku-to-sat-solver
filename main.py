            ...
            # full_solve: Solves all sudokus from an input file
                ...
                for path in path_list:
                    ...
                    if output == (0):
                        ...
                        all_digits = [
                            int(x.strip("v ")) + 1
                            for x in re.findall("v -?\d+", output)
                        ]
                ...
                with open("report-" + sys.argv[2] + ".txt", "w") as report:
                    ...
                    for path in path_list:
                        ...
                        if output == (0):
                            ...
                            our_time = round(end - start, 6)
                            percent_diff = round(
                                abs(zchaff_time - our_time) / zchaff_time, 2
                            )
                            result = (
                                "Our solver solved in "
                                + str(our_time)
                                + " seconds, "
                                + str(percent_diff)
                                + "% of zChaff\n"
                    ...
                    plt.plot(
                        list(range(i)), zchaff_times, label="zChaff", color="orange"
                    )
                    plt.xlabel("Sudoku #")
