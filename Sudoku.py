...
class Sudoku:
    def __init__(self, as_string="", order=3):
        if as_string == "":
            ...
            return
        self.grid = np.array(np.array_split(as_string, order ** 2)).reshape(
            (order ** 2, order ** 2)
        )
        self.order = order
    ...
    def solution_from_sat(self, sat_string):
        ...
        for digit in sat_string:
            if digit > 0:
                ...
                digit_string += str(d)
        self.grid = np.array(
            np.array_split(list(digit_string), self.order ** 2)
        ).reshape((self.order ** 2, self.order ** 2))
        return self.grid
    ...
    def to_sat(self):
        ...
        for coord in completeness_clauses.keys():
            unicity_clauses[coord] = list(
                itertools.combinations(completeness_clauses[coord], 2)
            )
        assert (
            len(unicity_clauses) * len(unicity_clauses[(0, 0)]) == (N ** 8 - N ** 6) / 2
        )
        ...
        for column in range(N ** 2):
            ...
            column_validity_dict[column] = [
                array.tolist()
                for array in np.array_split(column_validity_dict[column], N ** 2)
            ]
        ...
        for row in range(N ** 2):
            ...
            row_validity_dict[row] = [
                array.tolist()
                for array in np.array_split(row_validity_dict[row], N ** 2)
            ]
        ...
        for section in list(itertools.product([0, 1, 2], [0, 1, 2])):
            ...
            square_validity_dict[section] = [
                array.tolist()
                for array in np.array_split(square_validity_dict[section], N ** 2)
            ]
        ...
        assert (
            num_column_clauses
            == int(factorial(N ** 2) / factorial(2) / factorial(N ** 2 - 2)) * N ** 2
        )
        assert (
            num_row_clauses
            == int(factorial(N ** 2) / factorial(2) / factorial(N ** 2 - 2)) * N ** 2
        ...
        assert (
            num_column_clauses + num_row_clauses + num_square_clauses
        ) * N ** 2 == 3 * (N ** 8 - N ** 6) / 2
        ...
        for index in zip(list(non_zeros[0]), list(non_zeros[1])):
            sat_variable = [
                index[0] * N ** 4 + index[1] * N ** 2 + self.grid[index[0], index[1]]
            ]
        ...
        num_column_clauses = len(column_validity_clauses[0]) * len(
            column_validity_clauses
        )
        num_row_clauses = len(row_validity_clauses[0]) * len(row_validity_clauses)
        num_square_clauses = len(square_validity_clauses[(0, 0)]) * len(
            square_validity_clauses
        )
        ...
        for elem in column_validity_clauses.values():
            for clause in elem:
                demorgan = [
                    "-" + str(x[0] + 1) + " " + "-" + str(x[1] + 1)
                    for x in list(itertools.permutations(clause))
                ]
